"""
足球数据导入脚本
从 football_data.xlsx 导入历史比赛数据到数据库
"""
...
def import_football_data(excel_file: str, home_team_name: str = "朋队", dry_run: bool = False):
    """
    导入足球数据

    Args:
        excel_file: Excel 文件路径
        home_team_name: 主队名称（默认"朋队"）
    ...
    try:
            ...
            for match in matches_preview:
                ...
                if match['players']:
                    ...
                    if len(match['players']) > 5:
                        print(f"      ... 等 {len(match['players'])} 位球员出勤")

            print("\n" + "=" * 70)
            print("✅ 干运行完成！以上是数据预览。")
            print(f"   如需实际导入，请运行: python import_football_data.py")
            print("=" * 70)
            return
        ...
        home_team = get_or_create_team(db, home_team_name, founded_year=2024)
        ...
        for idx, row in data_df.iterrows():
            ...
            match_date = excel_date_to_serial(date_value)
            if not match_date:
                print(f"   ⚠️  行 {idx+2}: 无效日期 {date_value}，跳过")
                continue
            ...
            if existing_match:
                ...
                match = Match(
                    ...
                    notes=f"从 football_data.xlsx 导入"
                )
                db.add(match)
                db.commit()
                db.refresh(match)
...
def get_or_create_team(db: Session, team_name: str, founded_year: int = None) -> Team:
    ...
    if not team:
        team = Team(
            name=team_name,
            description=f"从历史数据导入的球队：{team_name}",
...
示例:
  # 干运行模式（查看数据但不导入）
  python import_football_data.py --dry-run

  # 正式导入
  python import_football_data.py

  # 指定自定义参数
  python import_football_data.py --file data.xlsx --team 我的球队
        """
    )

    parser.add_argument("--file", default="football_data.xlsx", help="Excel 文件路径（默认：football_data.xlsx）")
    parser.add_argument("--team", default="朋队", help="主队名称（默认：朋队）")
    parser.add_argument("--dry-run", action="store_true", help="干运行模式：只读取数据不实际导入")

    args = parser.parse_args()

    import_football_data(args.file, args.team, args.dry_run)
